...
const ProductSummary = () => {
    ...
    }, [dispatch, user, addressList.length]); // Include addressList.length in dependencies

    useEffect(() => {
        if (addressList.length) {
            dispatch(setSelectedAddress(addressList[0]));
        }
    }, [dispatch, addressList.length, addressList]); 
    ...
    const handelSelectedAddress = (_id) => {
        const address = addressList.find((elem) => elem._id === _id);
    ...
    const loginAlertUI = (
        <div>
            <Link to={`/login/${product && '?redirect=/product/' + product._id}`} className='btn btn-sm btn-warning'>
                Login
            </Link>
        </div>
    );
        ...
        <>
            <div className="col-lg-3 p-4 mt-3 border">
                ...
                <p className="text-muted small link-underline-hover" onClick={handelOpenAddressList}>
                    {user ? (
                        <small>
                            <i className="fas fa-map-marker-alt"></i> {selectedAddress ? `Deliver to ${selectedAddress.name.substring(0, selectedAddress.name.indexOf(" "))} - ${selectedAddress.city} ${selectedAddress.pinCode}` : <span>Select delivery location</span>}
                        </small>
                    ) : (
                ...
                <p className="text-muted">
                    ...
                    <select onChange={handelChangeQuantity} value={cartQuantity} style={{ margin: "0 0 0 4px" }} name="quantity" id="quantity">
                        {[...Array(10).keys()].map((i) => (
                ...
                <p className="card-text">
                    {user && user._id === product.user ? (
                        <Link to={`/nursery/plant/update/${product._id}`} style={{ width: '100%' }} className='btn btn-primary'>
                            Edit Your Plants
                        </Link>
                    ) : cart ? (
                        cart.quantity === cartQuantity ? (
                            <Link style={{ width: '100%' }} to={`/cart`} className='btn btn-secondary'>
                                Go To Cart
                            </Link>
                        ) : (
                            <button style={{ width: '100%' }} onClick={handleUpdateCart} className='btn btn-primary'>
                                Update Your Cart
                            </button>
                        )
                    ) : (
                        user && (
                            <button onClick={handleAddToCart} style={{ width: '100%' }} className='btn btn-primary'>
                                Add to Cart
                            </button>
                        )
                    )}
                    {!user && alert("Sign in to add plants to cart", (
                ...
                <p className="card-text">
                    {user ? (
                        !(user._id === product.user) && (
                            <button onClick={handelBuyProduct} style={{ width: '100%' }} className='btn btn-success'>
                                Order Now
                            </button>
                        )
                    ) : (
                        alert("Sign in to buy this plant", (
            ...
            {viewAddressList && (
                <AddressList
                    ...
                    setViewAddressList={setViewAddressList}
                    viewAddressList={viewAddressList}
                    redirect={`/${product && '?redirect=/product/' + product._id}`}
                />
            )}
        </>
    );
